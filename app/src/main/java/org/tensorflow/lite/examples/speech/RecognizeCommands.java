...
public class RecognizeCommands {
  ...
  public RecognitionResult processLatestResults(float[] currentResults, long currentTimeMS) {
    ...
    Log.v("currentResults", "currentResults"+ currentResults);
    ...
    for (Pair<Long, float[]> previousResult : previousResults) {
      ...
      Log.v("previousResult", "previousResult" + previousResult);
      // previousResult : {time, encode(speechtext+confidence)}
      Log.v("scoresTensor", "scoresTensor" + scoresTensor);
      int i = 0;
    ...
    //for(int k=0; k < sortedAverageScores.length; ++k) {
     // Log.v("RECCOMMAND", "1.sortedAverageScores" + sortedAverageScores[k]);
    //}

    // See if the latest top score is enough to trigger a detection.
    final int currentTopIndex = sortedAverageScores[0].index;
    Log.v("RECCOMMAND", "2.currentTopIndex" + sortedAverageScores[0].index );
    final String currentTopLabel = labels.get(currentTopIndex);
    Log.v("RECCOMMAND", "3.currentTopLabel" + labels.get(currentTopIndex) );
    final float currentTopScore = sortedAverageScores[0].score;
    Log.v("RECCOMMAND", "4.currentTopScore" + sortedAverageScores[0].score );
